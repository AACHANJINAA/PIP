...
CGameTimer::CGameTimer() {
	if (::QueryPerformanceFrequency((LARGE_INTEGER*)&m_nPerformanceFrequency))
	{
		...
		::QueryPerformanceCounter((LARGE_INTEGER*)&m_nLastTime); m_fTimeScale = 1.0f / m_nPerformanceFrequency;
	}
	else {
...
void CGameTimer::Tick(float fLockFPS) {
	...
	float fTimeElapsed = (m_nCurrentTime - m_nLastTime) * m_fTimeScale; if (fLockFPS > 0.0f)
	{
	...
	m_nFramesPerSecond++;
	m_fFPSTimeElapsed += fTimeElapsed; if (m_fFPSTimeElapsed > 1.0f)
	{
...
void CGameTimer::Reset() {
	...
	::QueryPerformanceCounter((LARGE_INTEGER*)&nPerformanceCounter); m_nLastTime = nPerformanceCounter;
	m_nCurrentTime = nPerformanceCounter; m_bStopped = false;
}
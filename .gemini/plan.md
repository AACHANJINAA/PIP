...
### 1.1. 클라이언트-서버 아키텍처 다이어그램
```mermaid
graph TD
    subgraph Client [Client (Custom DX12 Engine)]
        A[Game Loop / Logic] --> B[Scene / GameObject]
        B --> C[Renderer (DX12 PBR)]
        A --> D[Network Manager (TCP/IP)]
        A --> E[Resource Manager (GLTF/Texture)]
    end

    subgraph Server [Dedicated Server (IOCP)]
        F[Accept Thread] --> G[Session Manager]
        G --> H[Worker Threads / Logic Workers]
        H --> I[Room / World Logic]
        I --> J[Physics (Jolt) & AI]
        H --> K[DB Thread / MSSQL]
    end

    D <-->|Binary Protocol / State Sync / Dead Reckoning| G
```
...
- **비동기 로딩**: `std::async` 또는 전용 `IO Thread`를 구성하여 씬 전환 및 동적 스폰 시 메인 스레드 블로킹(프레임 드랍) 방지.
- **GLTF/GLB 파싱**: `tinygltf` 라이브러리를 활용하여 Mesh, Material, Skeleton/Animation 데이터 추출 후 엔진 포맷으로 변환.
...
### 3.1. IOCP 기반 스레드 모델
- **Accept Thread**: 클라이언트 연결(Accept) 요청만 전담.
...
- **DB Thread**: 비동기 DB 작업(저장/불러오기)을 전담하는 Task Queue 형태의 워커 스레드.
...
- 런타임 중에는 데이터 컨테이너를 **Read-Only**로 접근하여 별도의 Mutex Lock 없이 매우 빠르게 조회할 수 있도록 구성.

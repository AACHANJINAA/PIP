...
public:
	ReadFBXMesh() {};
	ReadFBXMesh(const std::string str);
	virtual ~ReadFBXMesh();
...
private:
	...
	void ProcessNode(aiNode* node, const aiScene* scene, std::vector<IlluminatedVertex>& combined_vertices, std::vector<UINT>& combined_indices);
	// Assimp Mesh를 처리하여 정점/인덱스 데이터를 추출하는 함수
	void ProcessMesh(aiMesh* mesh, const aiScene* scene, std::vector<IlluminatedVertex>& combined_vertices, std::vector<UINT>& combined_indices);

...
ComPtr<ID3D12RootSignature> GltfRootSignatureGenerator::create(ID3D12Device* device)
{
    ...
    ranges[4].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 9, 0,
        D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND);     // t9~t10 (IBL)
...
ComPtr<ID3D12RootSignature> SkinnedRootSignatureGenerator::create(ID3D12Device* device)
{
    ...
    // IBL 텍스처 추가 (t9~t10)
    ranges[4].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 9, 0, D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND);
...
ComPtr<ID3D12RootSignature> TerrainRootSignatureGenerator::create(ID3D12Device* device)
{
    ...
    ranges[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 9, 0, D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND); // t9~t10(IBL)← 추가

...
void SceneManager::process_scene_change_if_requested(ID3D12Device* device ,ID3D12CommandAllocator* command_allocator , ID3D12GraphicsCommandList* command_list)
{
   ...
   // command_list->Reset(command_allocator, nullptr);
	...
	if (_currentScene->scene_name() == _networkWantSceneName)
	{
		...
		_networkWantSceneName = ""; // 씬 로딩 완료 후 요청 초기화
	}
    CLOG("Scene Loading Complete! Not Send Ready to Server. Scene: " << _requestedSceneName);
...
void SceneManager::generate_grass_for_all_landscapes(const std::string& layer_name, const std::string& grass_mesh_path)
{
	for (const auto& landscapeObj : _MainlandscapeObjects)
	{
		...
		auto terrain_loader = std::dynamic_pointer_cast<TerrainLoader>(terrainRenderComp->mesh());
		if (!terrain_loader) continue;
